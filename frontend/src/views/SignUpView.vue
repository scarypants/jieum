...
export default {
  ...
  methods: {
    async fetchUsersInfo() {
      try {
        ...
        this.users = response.data
          ...
          this.error = error.message ?? '사용자 목록를 가져오는데 실패했습니다.'
      } 
    },
    // 아이디 유효성 검사 로직
    validateId() {
      ...
      } else if (!idRegex.test(this.loginId)) {
    ...
    async checkIdDuplicate() {
      ...
      const found = this.users.find(option => option.loginId === this.loginId)
    ...
    async checkNicknameDuplicate() {
      ...
      const found = this.users.find(option => option.nickname === this.nickname)
