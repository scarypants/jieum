...
export class IdeaController {
     ...
     *              - name: sort
     *                in: query
     *                description: 정렬 기준 (조회수순, 스크랩순, 최신순)
     *                required: true
     *                schema:
     *                    type: string
     *                    example: latest
    ...
    static async getIdeas(req, res) {
        try {
            ...
            if (!sort.includes("views" || "scraps" || "latest")) {
    ...
    static async getMyIdeas(req, res) {
        try {
            const ideas = await IdeaDetailsModel.getByWriterId(req.authenticatedUser.id)
            res.status(200).json(ideas)
        } catch (error) {
            switch (error) {
                case "not found":
    ...
    static async createIdea(req, res) {
        try {
            ...
            // 유효성 검사
            if (!categoryId || !validator.isNumeric(categoryId)) {
